...
import { vandorList, getVandorById } from "../../helpers/AppHelper";
...
const Payment = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <p className="bg_5 text-white p-2">Success</p>
          {
          ...
          {/* <button onClick={() => handleTrack(row, '2')} className="bg_1 text-white btn btn-sm ms-2" disabled={`${row.track_id.props.track_id === 2 ? 'disabled' : ''}`} >Stock</button> */}
  ...
  const loadPaymentStock = async () => {
    await axios
      .get("/get-payment-stocks", {
        headers: {
